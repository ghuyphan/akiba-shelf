...
export const itemList = () => {
	...
	try {
		...
		merchItems.forEach(({ name, imageUrl }) => {
			const imgSize = ['small', 'medium', 'large'];
			pathList[`closeup/${name}`] = imageUrl;
			pathList[`closeup-bg/${name}`] = imageUrl;
			pathList[`icon/${name}`] = imageUrl;
			pathList[`splash-art/${name}`] = imageUrl;
			pathList[`lc/${name}`] = imageUrl;
			imgSize.forEach((size) => {
				pathList[`splash-art/${size}/${name}`] = imageUrl;
				pathList[`lc/${size}/${name}`] = imageUrl;
			});
		});
	} catch (e) {
		console.error('Error loading custom merch images in assets.js:', e);
	}
