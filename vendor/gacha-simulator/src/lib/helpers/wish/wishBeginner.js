...
const beginerWish = (rarity, beginnerData) => {
	const featured = beginnerData?.featured || {};
	const characters = Array.isArray(beginnerData?.characters) ? beginnerData.characters : [];
	let { character = 'noelle', vision = 'geo' } = featured;
	...
	if (rarity === 4) {
		const result = get4StarItem('beginner', null, null, characters);
