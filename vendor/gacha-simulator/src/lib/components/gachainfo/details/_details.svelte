<script>
	...
	const { banner, bannerTitle, featured, items, merch = false, merchDescription = '' } = data;
...
<div class="content">
	{#if activeContent === 1}
		<PromotionalV2 data={{ featured, items, banner, bannerTitle, merch }} />
	{:else if activeContent === 2}
		<Description bannerType={banner} data={items} bannerName={bannerTitle} {merch} {merchDescription} />
	{:else if activeContent === 3}
		<List {drop5star} {drop4star} {drop3star} {banner} {merch} />
	{/if}
</div>
