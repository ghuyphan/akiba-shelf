<script>
	...
	let bannerTitle;
	let featured = [];
	let merchDescription = '';

	const setupMerchDetails = (selected) => {
		const pool = getMerchItems(selected.character?.id);
		const byRarity = (rarity) => pool.filter((item) => item.rarity === rarity);
		const rateUp = (rarity) => {
			const rarityPool = byRarity(rarity);
			const configured = rarityPool.filter((item) => item.featured);
			return configured.length ? configured : rarityPool.slice(0, rarity === 5 ? 1 : 3);
		};
		const promoted = pool.filter((item) => item.featured);
		const fiveStar = rateUp(5).length ? rateUp(5) : [promoted[0] || pool[0]].filter(Boolean);
		const fiveStarNames = new Set(fiveStar.map((item) => item.name));
		const fourStar = rateUp(4).length
			? rateUp(4)
			: [...promoted, ...pool].filter((item) => !fiveStarNames.has(item.name)).slice(0, 3);

		items = [
			{ rarity: 5, items: fiveStar },
	...
	$: if (isMerch && activeBanner.character?.id) setupMerchDetails(activeBanner);
...
{#if isMerch}
	<Details
		data={{ banner, bannerTitle, featured, items, merch: true, merchDescription }}
