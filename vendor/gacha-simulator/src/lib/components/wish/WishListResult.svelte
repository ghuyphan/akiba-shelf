							...
							>
								<div class="pic">
									...
									<div class="info">
										...
										{#if stelaFortuna || (type === 'character' && fateType)}
											<div class="bonus">
...
<style>
	...
	.masterless span {
		width: 100%;
		font-size: calc(2.2 / 100 * var(--card-height));
		content: '1';
		display: block;
		position: absolute;
		left: 0;
	...
	.merch-icon {
		...
		align-items: center;
		justify-content: center;
		width: calc(14 / 100 * var(--card-height));
		height: calc(14 / 100 * var(--card-height));
		max-width: 75%;
		border-radius: 50%;
		background: rgba(31, 43, 64, 0.78);
		border: 1px solid rgba(255, 255, 255, 0.8);
		box-shadow: 0 0 0.5rem rgba(101, 187, 246, 0.8);
	}
	...
	.merch-kind {
		margin-top: calc(1 / 100 * var(--card-height));
		padding: 0 calc(1.5 / 100 * var(--card-height));
		border-radius: 999px;
		background: rgba(31, 43, 64, 0.82);
		color: #fff4c4;
