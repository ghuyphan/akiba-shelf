...
<div class="wish-result" style="background-image: url({splashBG});">
	...
	{:else}
		...
		>
			...
			{#each list as { name, rarity, weaponType, type, vision, fateType, fateQty, stelaFortuna, outfit, isMerch }, i}
				{#if activeIndex === i}
					<div class="splatter star{rarity}" style={splatterStyle}>
						...
						{#if isMerch}
							<div class="splash-art anim merch-result star{rarity}">
								<img src={$assets[`splash-art/${name}`]} alt={name} />
							</div>
						{:else if type === 'weapon'}
...
<style>
	...
	:global(.mobile) .merch-result {
		...
		transform: scale(1) translate(8%, -2%);
	}
	...
	.merch-result.anim {
		filter: brightness(0) opacity(0);
		animation: splashartMerch forwards 1.5s 1;
	}

	:global(.mobile) .merch-result.anim {
		animation: splashartMerchMobile forwards 1.5s 1;
	}
	...
	.white-flash {
		...
		z-index: 1000;
		pointer-events: none;
		animation: whiteFlash 0.6s cubic-bezier(0.25, 1, 0.5, 1) forwards;
	}
	...
	@keyframes splashartMerchMobile {
		0% {
			transform: scale(5) translate(0, -10%);
			filter: opacity(0) brightness(0);
		}
		20% {
			transform: scale(1) translate(6%, -2%);
			filter: opacity(1) brightness(0);
		}
		75% {
			transform: scale(1) translate(6%, -2%);
