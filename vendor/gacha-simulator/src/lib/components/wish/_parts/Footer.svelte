...
<div id="footer" style="width: 100%; height: 100%">
	...
	<div class="row">
		<div class="left menu-button">
			...
			<ButtonGeneral on:click={() => changePage('inventory')}>{$t('inventory.text')}</ButtonGeneral>
			<ButtonGeneral on:click={() => changePage('history')}>{$t('history.text')}</ButtonGeneral>
		</div>
		<div class="right roll-button">
			...
			>
				...
				<div class="bottom">
					<Icon type={fateType} />
					<span style="margin-left: 7px" class:red={fateQty < multiRollPrice && !$unlimitedFates}>
						x {multiRollPrice}
					</span>
				</div>
			</button>
