...
- HSR (warp): every banner has 4 display slots. Event banners require exactly 1 featured 5★ primary plus 3 featured 4★ rate-ups; standard banners may leave all featured slots empty. Kind matters: character banners only feature character entries; Light Cone banners only feature non-character (lightcone/weapon) entries. `vendor/hsr-simulator/src/lib/helpers/banner-loader.js` slices featured items to `display_limit`, picks the 5★ primary, and shows the 3 uniquely named 4★ rate-ups on the banner card.
