-- Route public checkout through the Edge Function so abuse controls can use
-- request metadata while the existing create_order transaction stays intact.

create table private.checkout_reservation_clients (
  order_id uuid primary key references public.orders(id) on delete cascade,
  client_request_id uuid not null unique,
  shop_id uuid not null references public.shops(id) on delete cascade,
  fingerprint_hash text not null,
  created_at timestamptz not null default now(),
  constraint checkout_reservation_clients_fingerprint_check
    check (fingerprint_hash ~ '^[0-9a-f]{64}$')
);

revoke all on private.checkout_reservation_clients
from public, anon, authenticated;

create index checkout_reservation_clients_rate_idx
  on private.checkout_reservation_clients (
    shop_id,
    fingerprint_hash,
    created_at desc
  );
...
begin
  ...
  -- Serialize quota checks for one client and shop. Retries with the same
  -- request ID bypass quotas and remain governed by create_order idempotency.
  ...
  if v_order.id is null then
    raise exception 'Order creation returned no result';
  end if;
...
-- Remove historical broad table privileges and expose only staff UI columns.
...
-- Cover the composite product foreign key reported by the linked advisor.
create index gacha_pool_entries_shop_product_idx
  on public.gacha_pool_entries (shop_id, product_id);

-- Browser uploads are normalized to WebP before reaching Storage.
update storage.buckets
set allowed_mime_types = array['image/webp'],
    file_size_limit = 5 * 1024 * 1024
where id in ('product-images', 'payment-qr');
