-- Reimagined gacha persistence: one transactional publish RPC replaces the
-- previous 7-call client orchestration. Published relational tables stay the
-- public read projection; gacha_game_configs becomes the only admin draft
-- store. Existing live configurations are backfilled into drafts so shops
-- keep an editable copy of what is currently published.
...
-- Backfill drafts from the currently published configuration so existing
-- shops open the redesigned editor with their live setup preloaded.
