-- Keep the order-queue status badges scoped to the same local-day window as
-- the visible orders. Optional bounds preserve the existing all-time call.
...
revoke all on function public.get_order_status_counts(uuid, timestamptz, timestamptz)
  from public, anon, authenticated;
grant execute on function public.get_order_status_counts(uuid, timestamptz, timestamptz)
  to authenticated;
