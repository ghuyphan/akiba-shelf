        ...
        <Modal title={copy.scanPay} isOpen={isOpen} onClose={onClose} className="payment-modal payment-qr-modal-redesign checkout-flow-modal" mobileSheet closeLabel={copy.closeModal}>
          ...
          <form onSubmit={handlePlaceOrder} className="order-confirm-layout" noValidate>
            ...
            <div className="order-confirm-side">
              <label className="order-confirm-name"><span>{copy.pickupName}</span><div><UserRound size={18} aria-hidden="true" /><input id="pickup-name" type="text" aria-label={copy.pickupName} placeholder={copy.pickupPlaceholder} value={customerName} aria-invalid={Boolean(customerNameError) || undefined} aria-describedby={customerNameError ? "pickup-name-error" : undefined} onChange={(event) => { setCustomerName(event.target.value); if (customerNameError) setCustomerNameError(""); }} maxLength={30} /></div>{customerNameError ? <small className="order-confirm-name-error" id="pickup-name-error" role="alert">{customerNameError}</small> : <small>{copy.pickupHint}</small>}</label>
