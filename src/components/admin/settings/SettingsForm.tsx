  ...
  return (
    ...
    >
      <form className="admin-form" onSubmit={handleSubmit}>
        <AdminFormError
          error={error}
          fallback="Could not save booth settings"
          title="Could not save booth settings"
          onDismiss={() => setError("")}
        />
        {!isEditing && (
          <div className="admin-readout">
            <span>
              <Store size={16} />
              <small>{t("Booth")}</small>
              <strong>{draft.booth_name || t("Not set")}</strong>
            </span>
            <span>
              <MapPin size={16} />
              <small>{t("Code")}</small>
              <strong>{draft.booth_code || t("Not set")}</strong>
            </span>
            <span>
              <Clock3 size={16} />
              <small>{t("Hours")}</small>
              <strong>{draft.open_hours || t("Not set")}</strong>
            </span>
          </div>
        )}
        ...
        <section className="admin-form-section">
          <div className="admin-form-section-heading">
            ...
            <div>
              <h3>{t("Booth identity")}</h3>
              <p>{t("Name, code, logo, hours, and location.")}</p>
            </div>
          </div>
          <div className="admin-logo-editor">
            <div className="admin-logo-preview">
              {draft.logo_url ? (
                <img src={draft.logo_url} alt={t("Booth logo")} />
              ) : (
                <Store size={25} />
              )}
            </div>
          ...
          <div className="form-grid">
            <Field label={t("Booth name")}>
              <TextInput
                ...
                disabled={!isEditing}
                onChange={(event) =>
                  setDraft({ ...draft, booth_name: event.target.value })
                }
              />
            </Field>
            <Field label={t("Booth code")}>
              <TextInput
                ...
                disabled={!isEditing}
                onChange={(event) =>
                  setDraft({ ...draft, booth_code: event.target.value })
                }
              />
            </Field>
            <Field label={t("Open hours")}>
              <TextInput
                ...
                disabled={!isEditing}
                onChange={(event) =>
                  setDraft({ ...draft, open_hours: event.target.value })
                }
              />
            </Field>
        ...
        <section className="admin-form-section">
          <div className="admin-form-section-heading">
            ...
            <div>
              <h3>{t("Store copy")}</h3>
              <p>{t("Customer-facing title and description.")}</p>
            </div>
          </div>
        ...
        <section className="admin-form-section">
          <div className="admin-form-section-heading">
            ...
            <div>
              ...
              <p>{t("Make this palette completely yours.")}</p>
            </div>
          </div>
        ...
        <section className="admin-form-section">
          <div className="admin-form-section-heading">
            ...
            <div>
              ...
              <p>{t("Links shown in booth information.")}</p>
            </div>
          </div>
          <SocialLinkFields
            settings={draft}
            ...
            onChange={setDraft}
          />
          <Field label={t("QR center logo")}>
            <TextInput
              value={draft.social_qr_logo_url ?? ""}
              disabled={!isEditing}
              onChange={(event) =>
                setDraft({ ...draft, social_qr_logo_url: event.target.value })
              }
            />
          </Field>
          <div className="admin-social-preview">
            <span>
              {draft.social_qr_logo_url ? (
                <img src={draft.social_qr_logo_url} alt={t("Social QR logo")} />
              ) : (
                <>
                  <SocialBrandIcon platform="Instagram" size={17} />
                  <SocialBrandIcon platform="Threads" size={17} />
                  <SocialBrandIcon platform="YouTube" size={17} />
                </>
              )}
            </span>
            <div>
              <strong>{t("Shared QR logo")}</strong>
              <small>{t("Used in the center of every social QR code.")}</small>
            </div>
            {isEditing && (
              <ImageUpload
                shopId={shopId}
