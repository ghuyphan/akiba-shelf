...
export function ProductList({
  ...
  return (
    <AdminCard
      title={t("Products")}
      description={t("{{count}} catalog items", { count: products.length })}
      icon={<Boxes size={18} />}
      ...
      <div className="admin-product-list admin-scroll-list">
        {visibleProducts.length === 0 && (
          <EmptyState
            ...
            message={
              loading
                ? t("Fetching the latest catalog and stock levels.")
                : products.length === 0
                  ? t("Create your first item to start filling the booth.")
                  : t("Adjust your search or return to all products.")
            }
            action={
            ...
            >
              <span className="admin-product-thumb">
                {primaryImage ? (
                  ...
                  <ImageIcon
                    size={24}
                    className="admin-product-thumb-placeholder"
                  />
                )}
              </span>
              <span className="admin-product-copy">
                <span className="admin-product-title-row">
                  <strong>{product.name || t("Untitled item")}</strong>
                  {product.featured && (
                  ...
                  {!product.active && (
                    <span className="admin-status-pill hidden">
                      {t("Hidden")}
                    </span>
                  )}
                </span>
                <small>
                  {product.item_code || t("No code")} ·{" "}
                  {formatVnd(getProductPrice(product))}
                  {isProductOnSale(product) ? ` · ${t("Sale")}` : ""}
                </small>
                <span className={`admin-stock-pill ${stockTone}`}>
                  <i />
                  {product.quantity_available <= 0
                    ? t("Sold out")
                    : t("{{count}} in stock", {
