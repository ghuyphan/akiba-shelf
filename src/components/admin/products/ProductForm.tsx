...
export function ProductForm({
  ...
  const selectedFeaturedCount =
    featuredCount - Number(product.featured) + Number(draft.featured);
  const featuredLimitReached =
    featuredCount >= MAX_FEATURED_PRODUCTS && !product.featured;
  const featuredHint =
    selectedFeaturedCount >= MAX_FEATURED_PRODUCTS
      ? t("All {{limit}} featured slots are used.", {
  ...
  function removeImage(index: number) {
    ...
    const nextDraft = {
      ...
      image_variants:
        variants.length === draft.images.filter(Boolean).length
          ? variants.filter((_, imageIndex) => imageIndex !== index)
          : variants,
      image_paths:
        variants.length === draft.images.filter(Boolean).length
          ? paths.filter((_, pathIndex) => Math.floor(pathIndex / 2) !== index)
          : paths,
    };
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    const quantity = Math.max(
      0,
      Math.floor(Number(draft.quantity_available) || 0),
    );
    ...
    await runSave(async () => {
      await onSave({
        ...
        id:
          normalizedDraft.id ||
          normalizeSlug(`${normalizedDraft.item_code}-${normalizedDraft.name}`),
      });
      didSave = true;
    }).catch(() => undefined);
  ...
  return (
    <AdminCard
      title={
        isNewProduct ? t("Create product") : draft.name || t("Product details")
      }
      description={
        isNewProduct
          ? t("Add the essentials first. You can refine the listing later.")
          : `${draft.item_code} · ${draft.category}`
      }
    ...
    >
      <form
        ref={formRef}
        className={`admin-form admin-product-form ${isEditing ? "is-editing" : "is-readonly"}`}
      ...
      >
        ...
        {!isEditing && (
          <div className="admin-readout">
            <span>
              <Tags size={16} />
              <small>{t("Price")}</small>
              <strong>{formatVnd(getProductPrice(draft))}</strong>
              {isProductOnSale(draft) && (
                <del>{formatVnd(draft.price_vnd)}</del>
              )}
            </span>
            <span>
              <Boxes size={16} />
            ...
            <span>
              ...
              <strong>{t(draft.active ? "Live" : "Hidden")}</strong>
            </span>
          </div>
        )}
        ...
        <section className="admin-form-section">
          <div className="admin-form-section-heading">
            ...
            <div>
              ...
              <p>{t("The information customers use to identify this item.")}</p>
            </div>
          </div>
          <div className="form-grid">
            <Field
              label={t("Product name · Required")}
              error={
                getFieldError("name") ? t(getFieldError("name")!) : undefined
              }
            >
              <TextInput
                placeholder={t("e.g. Moonlight acrylic stand")}
                value={draft.name}
                disabled={!isEditing}
            ...
            <Field
              label={t("Item code · Required")}
              hint={
                getFieldError("item_code")
                  ? undefined
                  : t("A short unique code used by staff.")
              }
              error={
                getFieldError("item_code")
                  ? t(getFieldError("item_code")!)
                  : undefined
              }
            >
              <TextInput
                ...
                disabled={!isEditing}
                aria-invalid={Boolean(getFieldError("item_code"))}
                onFocus={startTextEditSession}
                onChange={(event) =>
                  setField("item_code", event.target.value.toUpperCase())
                }
              />
            </Field>
            <Field
              label={t("Collection")}
              hint={t("Optional grouping, such as Spring 2026.")}
            >
              <TextInput
                placeholder={t("e.g. Starry Days")}
            ...
            <Field
              ...
              error={
                getFieldError("category")
                  ? t(getFieldError("category")!)
                  : undefined
              }
            >
              <TextInput
                placeholder={t("e.g. Acrylic, Print, Apparel")}
          ...
          >
            <TextArea
              maxLength={500}
              placeholder={t("What should customers know about this item?")}
        ...
        <section className="admin-form-section">
          <div className="admin-form-section-heading">
            ...
            <div>
              <h3>{t("Price & availability")}</h3>
              <p>
                {t("Stock status updates automatically from the quantity.")}
              </p>
            </div>
          </div>
          <div className="form-grid">
            <Field
              label={t("Regular price · Required")}
              error={
                getFieldError("price_vnd")
                  ? t(getFieldError("price_vnd")!)
                  : undefined
              }
            >
              <div className="admin-input-affix">
                <TextInput
                  ...
                  disabled={!isEditing}
                  aria-invalid={Boolean(getFieldError("price_vnd"))}
                  onFocus={startTextEditSession}
                  onChange={(event) => {
                    const raw = event.target.value.replace(/\D/g, "");
                    setField("price_vnd", raw ? Number(raw) : 0);
                  }}
                />
                <span>VND</span>
              </div>
            </Field>
            {draft.sale_price_vnd != null && (
              <Field
                label={t("Sale price · Required")}
                hint={
                  draft.price_vnd > 0 && draft.sale_price_vnd < draft.price_vnd
                    ? t("Customers save {{percent}}%.", {
                        percent: getProductDiscountPercent(draft),
                      })
                    : t("Must be lower than the regular price.")
                }
                error={
                  getFieldError("sale_price_vnd")
                    ? t(getFieldError("sale_price_vnd")!)
                    : undefined
                }
              >
                <div className="admin-input-affix">
                  <TextInput
                    ...
                    inputMode="numeric"
                    placeholder="0"
                    value={formatDisplayPrice(draft.sale_price_vnd)}
                    disabled={!isEditing}
                    aria-invalid={Boolean(getFieldError("sale_price_vnd"))}
                    onFocus={startTextEditSession}
                    onChange={(event) => {
                      const raw = event.target.value.replace(/\D/g, "");
                      setField("sale_price_vnd", raw ? Number(raw) : 0);
                    }}
                  />
                  <span>VND</span>
                </div>
            ...
            <Field
              label={t("Quantity")}
              error={
                getFieldError("quantity_available")
                  ? t(getFieldError("quantity_available")!)
                  : undefined
              }
            >
              <QuantityInput
                ...
                disabled={!isEditing}
                invalid={Boolean(getFieldError("quantity_available"))}
                onChange={setQuantity}
              />
            </Field>
            <Field
              label={t("Product badge")}
              hint={t("Optional label shown on product artwork.")}
            >
              <SelectMenu
                label={t("Product badge")}
                value={draft.badge ?? ""}
                ...
                options={[
                  ...
                  ...(hasLegacyBadge
                    ? [{ value: draft.badge ?? "", label: draft.badge ?? "" }]
                    : []),
              ...
              />
              {draft.badge && (
                <div className="admin-badge-customizer">
                  <ColorPicker
                    compact
                    label={t("Badge color")}
                    value={draft.badge_color || "#5f8d55"}
                    disabled={!isEditing}
                    onChange={(value) => setField("badge_color", value)}
                  />
                  <span
                    className="admin-badge-preview"
                    style={{ background: draft.badge_color || "#5f8d55" }}
                  >
                    <Sparkles size={12} />
                    {t(draft.badge)}
                  </span>
          ...
          <div className="admin-switch-row">
            <label htmlFor="product-sale-toggle">
              <span>
                <strong>{t("Put this item on sale")}</strong>
                <small>
                  {t(
              ...
              <input
                id="product-sale-toggle"
                type="checkbox"
                aria-label={t("Put this item on sale")}
                checked={draft.sale_price_vnd != null}
                disabled={!isEditing}
                onChange={(event) =>
                  setField(
                    "sale_price_vnd",
                    event.target.checked
                      ? Math.max(0, draft.price_vnd - 10_000)
                      : null,
                  )
                }
              />
            </label>
            <label htmlFor="product-featured-toggle">
              <span>
                <strong>{t("Feature this item")}</strong>
                <small>{featuredHint}</small>
              </span>
              <input
                id="product-featured-toggle"
                type="checkbox"
                aria-label={t("Feature this item")}
                checked={draft.featured}
                disabled={!isEditing || featuredLimitReached}
                onChange={(event) => setField("featured", event.target.checked)}
              />
            </label>
            <label htmlFor="product-visible-toggle">
              <span>
                <strong>{t("Visible in storefront")}</strong>
                <small>{t("Customers can find and purchase this item.")}</small>
              </span>
              <input
                id="product-visible-toggle"
                type="checkbox"
                aria-label={t("Visible in storefront")}
        ...
        <section className="admin-form-section">
          <div className="admin-form-section-heading">
            ...
            <div>
              ...
              <p>
                {t("Add up to four images. The first image becomes the cover.")}
              </p>
            </div>
          </div>
          <div className="admin-image-gallery">
            {images.map((image, index) => (
              <div className="admin-image-tile" key={`${image}-${index}`}>
                <img
                  src={image}
                  alt={`${draft.name || t("Product")} ${index + 1}`}
                />
            ...
            {images.length === 0 && (
              <div className="admin-image-empty">
                <ImageIcon size={25} />
                <span>{t("No product images yet")}</span>
              </div>
            )}
          ...
          {getFieldError("images") && (
            <div className="field-error-msg admin-gallery-error">
              {t(getFieldError("images")!)}
            </div>
          )}
        </section>
