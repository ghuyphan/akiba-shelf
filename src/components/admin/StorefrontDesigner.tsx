        ...
        {sidebarOpen && <>
          ...
          <div className="builder-inspector">
            ...
            {tab === "style" && <>
              ...
              <div className="designer-locale"><Languages size={17} /><span><strong>{copy.designer.storefrontLanguage}</strong><small>{copy.designer.storefrontLanguageHint}</small></span><div><button type="button" className={(draft.catalog_locale ?? "en") === "en" ? "active" : ""} onClick={() => update("catalog_locale", "en")}>EN</button><button type="button" className={draft.catalog_locale === "vi" ? "active" : ""} onClick={() => update("catalog_locale", "vi")}>VI</button></div></div>
            </>}
        ...
        >
          <div className="designer-preview-frame" style={{ width: `${(device === "phone" ? 390 : 1380) * previewScale}px`, minHeight: `${(device === "phone" ? 844 : 1120) * previewScale}px` }}>
            ...
            {previewDocument && createPortal(
              <CatalogLocaleProvider locale={draft.catalog_locale ?? "en"} targetDocument={previewDocument} restorePlatformLocale={false}>
