...
export function OrderQueue({ orders, filter, counts, page, pageSize, total, loading, onFilterChange, onPageChange, onOrderUpdated }: OrderQueueProps) {
  ...
  async function handleConfirm(orderId: string) {
    ...
    try { const result = await confirmOrderPayment(orderId); onOrderUpdated(); if (result.outcome !== "confirmed") { toast.info("This order was already handled by another staff member."); return false; } toast.success("Payment confirmed."); return true; }
  ...
  async function handleCancel(orderId: string) {
    ...
    try { const result = await cancelOrder(orderId); onOrderUpdated(); if (result.outcome !== "cancelled") toast.info("This order was already handled by another staff member."); else toast.success("Order cancelled and stock released."); }
...
function OrderCard({ order, isConfirming, isCancelling, onConfirm, onCancel }: OrderCardProps) {
  ...
  return (
    <article className={`admin-order-card status-${order.status}`}>
      ...
      {order.status === "pending" && <div className="admin-order-actions"><SwipeConfirmButton onConfirm={onConfirm} isConfirming={isConfirming} /><button type="button" className="admin-cancel-order" onClick={onCancel} disabled={isConfirming || isCancelling}><Ban size={15} />{isCancelling ? "Cancelling…" : "Cancel and release stock"}</button></div>}
    </article>
  );
}
