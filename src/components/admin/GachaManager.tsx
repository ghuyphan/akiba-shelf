...
type HistoryAction =
  ...
  | { type: "UPDATE_STATE"; updater: (curr: GachaState) => GachaState; pushHistory: boolean }
...
function historyReducer(state: HistoryStoreState, action: HistoryAction): HistoryStoreState {
  switch (action.type) {
    case "LOAD_STATE":
      return {
        past: [],
    ...
    case "UPDATE_STATE": {
      ...
      if (
        JSON.stringify(state.present.settings) === JSON.stringify(nextPresent.settings) &&
        JSON.stringify(state.present.banners) === JSON.stringify(nextPresent.banners) &&
        JSON.stringify(state.present.entries) === JSON.stringify(nextPresent.entries) &&
...
export function GachaManager({ shopId, shopSlug, products }: Props) {
  ...
  const [initialConfig, setInitialConfig] = useState<GachaState | null>(null);
  ...
  const hasChanges = useMemo(() => {
    if (!initialConfig || !historyState.present) return false;
    return (
      JSON.stringify(historyState.present.settings) !== JSON.stringify(initialConfig.settings) ||
      JSON.stringify(historyState.present.banners) !== JSON.stringify(initialConfig.banners) ||
      JSON.stringify(historyState.present.entries) !== JSON.stringify(initialConfig.entries)
    );
  }, [initialConfig, historyState.present]);
  ...
  const handleTextChange = useCallback((updater: (curr: GachaState) => GachaState) => {
  ...
  const setBanners = useCallback((
    ...
    updateState((curr) => ({
      ...curr,
      banners: typeof nextBanners === "function" ? nextBanners(curr.banners) : nextBanners,
    }), true);
  }, [updateState]);

  const setEntries = useCallback((
    nextEntries: GachaPoolEntry[] | ((curr: GachaPoolEntry[]) => GachaPoolEntry[])
  ) => {
    updateState((curr) => ({
      ...curr,
      entries: typeof nextEntries === "function" ? nextEntries(curr.entries) : nextEntries,
    }), true);
  }, [updateState]);

  const setSelectedBannerId = useCallback((id: string | ((curr: string) => string)) => {
    updateState((curr) => ({
      ...curr,
      selectedBannerId: typeof id === "function" ? id(curr.selectedBannerId) : id,
    }), false);
  }, [updateState]);

  const resetChanges = useCallback(() => {
    if (!initialConfig) return;
    dispatch({ type: "RESET", initialState: initialConfig });
  }, [initialConfig]);

  const stateRef = useRef({ past: historyState.past, future: historyState.future });
  useEffect(() => {
  ...
  const load = useCallback(async () => {
    ...
    const nextEntries = capFeaturedEntries(next.entries, nextBanners);
    const initialSelectedId = nextBanners[0]?.id ?? "";
    const config = {
      settings: next.settings,
      banners: nextBanners,
      entries: nextEntries,
      selectedBannerId: initialSelectedId,
    };
  ...
  function updateEntry(productId: string, changes: Partial<GachaPoolEntry>) {
    ...
    const updater = (curr: GachaState) => ({
      ...
      entries: curr.entries.map((entry) =>
        entry.banner_id === curr.selectedBannerId && entry.product_id === productId
          ? { ...entry, ...changes }
          : entry
      ),
    });
            ...
            <label className="gacha-toggle">
              <input
                ...
                onChange={(event) =>
                  updateState((curr) => ({
                    ...curr,
                    settings: { ...curr.settings, enabled: event.target.checked },
                  }), true)
                }
              />
