...
export function QrManager({ shopId, settings, onSave }: QrManagerProps) {
  ...
  useEffect(() => { if (error) { toast.error(error, "Could not save payment settings"); setError(""); } }, [error, setError, toast]);
  ...
  return (
    <AdminCard title="Payment & QR" description="Bank details and payment instructions." icon={<QrCode size={18} />} action={!isEditing ? <Button type="button" variant="secondary" icon={<Edit3 size={17} />} onClick={() => setIsEditing(true)}>Edit</Button> : undefined}>
      <form className="admin-form" onSubmit={handleSubmit}>
        ...
        <section className="admin-form-section">
          <div className="admin-form-section-heading"><span>01</span><div><h3>Bank account</h3><p>Used to generate each payment QR.</p></div></div>
        ...
        <section className="admin-form-section">
          <div className="admin-form-section-heading"><span><MessageSquareText size={15} /></span><div><h3>Transfer message</h3><p>Match payments to orders with tokens.</p></div></div>
