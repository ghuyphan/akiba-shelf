...
type ToastItem = Required<Pick<ToastInput, "message" | "variant">> &
  Omit<ToastInput, "message" | "variant"> & { id: string };
type ToastApi = {
...
const ToastRegionContext = createContext<HTMLElement | null>(null);
...
export function ToastProvider({
  ...
  const [localizedLabels, setLocalizedLabels] = useState<ToastLabels | null>(
    null,
  );
  ...
  const show = useCallback(
    (input: ToastInput) => {
      ...
      setToasts([toast]);
      const duration =
        input.duration ?? (toast.variant === "error" ? 6500 : 4000);
  ...
  return (
    <ToastContext.Provider value={api}>
      <ToastLabelsContext.Provider value={setLocalizedLabels}>
                ...
                {toasts.map((toast) => {
                  ...
                  return (
                    <div
                      ...
                      style={
                        {
                          "--toast-duration": `${toast.duration ?? (toast.variant === "error" ? 6500 : 4000)}ms`,
                        } as CSSProperties
                      }
                        ...
                        <strong>
                          {toast.title ??
                            (toast.variant === "success"
                              ? activeLabels.successTitle
                              : toast.variant === "error"
                                ? activeLabels.errorTitle
                                : activeLabels.infoTitle)}
                        </strong>
                        <p>{toast.message}</p>
                      </div>
                      <button
                        type="button"
                        aria-label={activeLabels.dismiss}
...
export function ToastStackPortal({ children }: { children: ReactNode }) {
  const region = useContext(ToastRegionContext);
