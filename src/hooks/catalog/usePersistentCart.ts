...
export function usePersistentCart(shopKey?: string) {
  ...
  const loadedShopKeyRef = useRef(shopKey);
  ...
  useEffect(() => {
    if (loadedShopKeyRef.current === shopKey) return;
    loadedShopKeyRef.current = shopKey;
  ...
  useEffect(() => {
    saveCart(cart, shopKey);
  }, [cart, shopKey]);

  const reconcileCart = useCallback(
    (products: Product[], authoritativeIds?: string[]) => {
      ...
      const next = cartRef.current
        .map((item) => {
          ...
          const quantity = Math.min(
            item.quantity,
            Math.max(0, product.quantity_available - rewardQuantity),
          );
