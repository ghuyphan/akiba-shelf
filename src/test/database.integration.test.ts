...
describe.skipIf(!run)("database concurrency", () => {
  ...
  beforeAll(async () => {
    ...
    const { error: shopError } = await admin.from("shops").insert({ id: shopId, name: "Concurrency", slug: shopSlug, created_by: staffUserId });
    if (shopError) throw shopError;
    const { error: memberError } = await admin.from("shop_members").insert({ shop_id: shopId, user_id: staffUserId, role: "staff" });
    if (memberError) throw memberError;
    const { error: productError } = await admin.from("products").insert({ id: productId, shop_id: shopId, name: "Concurrency", item_code: `C-${suffix}`, price_vnd: 12345, quantity_available: 5, category: "Test", active: true });
  ...
  afterAll(async () => {
    ...
    await admin.from("shop_members").delete().eq("shop_id", shopId).eq("user_id", staffUserId);
  ...
  it("serializes retries and prevents overselling", async () => {
    ...
    const args = { p_shop_slug: shopSlug, p_customer_name: `test-${suffix}-retry`, p_items: [{ product_id: productId, quantity: 2 }], p_client_request_id: requestId, p_recovery_token: token };
  ...
  it("allows only one terminal transition", async () => {
    ...
    const { data, error } = await customer.rpc("create_order", { p_shop_slug: shopSlug, p_customer_name: `test-${suffix}-terminal`, p_items: [{ product_id: productId, quantity: 1 }], p_client_request_id: crypto.randomUUID(), p_recovery_token: recoveryToken });
